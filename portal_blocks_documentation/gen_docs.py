...
for file in (project_dir / "docs_json").glob("*.json"):
    with file.open("r") as JSON_FILE, (build_dir / (file.stem + ".md")).open(
        "w"
    ) as DOC_FILE:
        doc = [
            f"""---
...
---"""
        ...
        doc.append(
            f"\n![{file.stem}](https://raw.githubusercontent.com/battlefield-portal-community/Image-CDN/main/portal_blocks/{file.stem}.png)"
            # noqa
        )
